...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let url = "Web-env.m3uyi3cjpb.us-east-1.elasticbeanstalk.com/hw8/getGoogleDetails?placeid=\(rows[indexPath.row].placeId!)"
        //print(url)
...
//                let addr1 = addressComponents[0]["short_name"]!
                let url = "Web-env.m3uyi3cjpb.us-east-1.elasticbeanstalk.com/hw8/getDetails?name=\(self.rows[self.selectedCell.row].name!)&address1=\(addressComponents[0]["short_name"]!!) \(addressComponents[1]["short_name"]!!)&city=\(city!)&state=\(state!)&country=US"
                print(url)
                Alamofire.request(url).responseJSON { response in
                    ...
                    if let json = response.result.value as? Array<AnyObject>{
                        print("yelp match")
                        print("yelp\(json)")
                        let url = "Web-env.m3uyi3cjpb.us-east-1.elasticbeanstalk.com/hw8/getReview?place_id=\(json[0]["id"]!)"
                        print(url)
                        Alamofire.request(url).responseJSON { response in
